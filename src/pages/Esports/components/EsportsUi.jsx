...
const EsportsUi = () => {
  return (
    <div className="py-5 px-4 md:px-12 bg-slate-100 h-screen overflow-hidden">
      <div className="flex justify-between gap-2 w-full h-full">
        {/* Sidebar (scrollable) */}
        <div className="w-full md:w-[80%] h-full flex gap-5 overflow-y-auto custom-scrollbar">
          <div className="hidden md:block w-[20%] h-full">
            <ESportsSidebar />
          </div>

          {/* Middle (scrollable) */}
          <div className="w-full md:w-[80%] h-full pr-2 min-w-0">
            <EsportsMiddle />
          </div>
        </div>

        {/* Right - Bet Slip (fixed) */}
        <div className="hidden md:block w-[20%] h-full overflow-hidden">
