  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-lg bg-white rounded-lg p-6 z-10">
        <h1 className="text-xl font-bold text-center mb-5">{language === "turkish" ? "HOŞ GELDİNİZ." : "WELCOME."}</h1>
        <p className="text-sm text-gray-500 text-center mb-6">{language === 'turkish' ? 'Aşağıdaki formu doldurarak hesabınızı oluşturmaya' : 'You can start creating your account by filling out the form below.'} <br />{language === 'turkish' ? "başlayabilirsiniz." : ''} </p>

        <form>
          <div className="mb-4">
            <input
              type="text"
              placeholder={language === 'turkish' ? "Kullanıcı adı" : 'Username'}
              className="w-full p-3 border border-gray-300 rounded"
            />
          </div>
          ...
          <div className="mb-2">
            <input
              type="email"
              placeholder={language === 'turkish' ? "E-posta" : 'Email'}
              className="w-full p-3 border border-gray-300 rounded"
            />
          </div>
          ...
          <p className="text-xs mb-4">
            {
              language === "turkish" ?
                "Hesabınızı aktif edebilmeniz için lütfen Email bilgisi girdiğinizden emin olunuz ve emailinize gelen linke tıklayınız onaylayınız." :
                "To activate your account, please make sure you entered a valid email and confirm it through the link sent."
            }
          </p>
          ...
          <div className="mb-4 relative">
            <input
              type={showConfirmPassword ? "text" : "password"}
              placeholder={language === 'turkish' ? "Yeni Parolayı Onayla" : 'Confirm New Password'}
          ...
          <p className="text-xs mb-4">
            {language === "turkish" ?
              "Hesabınızı aktif edebilmeniz için lütfen telefon numaranızın girdiğini olduğundan emin olunuz ve telefonunuza gelen kod ile hesabınızı onaylayınız." :
              "To activate your account, please ensure your phone number is entered and confirm using the code sent."
            }
          </p>
          <div className="mb-4">
            <select className="w-full p-3 border border-gray-300 rounded text-gray-500">
              <option>{language === "turkish" ? "Cinsiyet" : "Gender"}</option>
              <option>{language === "turkish" ? "Erkek" : "Male"}</option>
              <option>{language === "turkish" ? "Kadın" : "Female"}</option>
            </select>
          </div>
          ...
          <div className="mb-4">
            <input
              type="text"
              placeholder={language === 'turkish' ? "Ad" : 'First name'}
          ...
          <div className="mb-4">
            <input
              type="text"
              placeholder={language === 'turkish' ? "Soyad" : 'Last name'}
          ...
          <div className="mb-4 flex space-x-2">
            <input
              type="text"
              placeholder={language === 'turkish' ? "Gün" : 'Day'}
            ...
            <input
              type="text"
                            placeholder={language === 'turkish' ? "Ay" : 'Month'}

              className="w-1/3 p-3 border border-gray-300 rounded"
            />
            <input
              type="text"
                            placeholder={language === 'turkish' ? "Yıl" : 'Year'}
          ...
          <div className="mb-6">
            <textarea
                            placeholder={language === 'turkish' ? "Adres" : 'Address'}
              className="w-full p-3 border border-gray-300 rounded resize-none h-24"
            ></textarea>
          </div>
          ...
          <div className="mb-2 flex items-start">
            ...
            <p className="text-xs">
              {
                language==="turkish"
              ?"Bonus ve Sürpriz Hediyelerle ilgili Bilgi Mail ve SMS almak istiyorum.":
              "I want to receive promotional emails and SMS about bonuses and surprise gifts."
              }
              </p>
          </div>
          ...
          <button type="submit" className="w-full bg-purple-600 text-white py-3 rounded font-medium hover:bg-purple-700 transition">
           {
            language==="turkish"?" HESAP OLUŞTUR":"CREATE ACCOUNT"
           }
          </button>
        </form>
      </div>
